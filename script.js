...
let gameStartTime = 0;
let sessionToken = '';   // Her oyun için benzersiz token
...
function setStatus(text, mode) {
    ...
    statusIndicator.classList.remove('is-armed', 'is-live', 'is-done');
    if (mode) statusIndicator.classList.add(mode);
}
...
function startCountdown() {
    ...
    setStatus('HEDEF BELİRLENİYOR...', 'is-armed');
...
function startRealGame() {
    // Her oyun için benzersiz session token oluştur
    ...
    updateStats();
    timeDisplay.innerText = timeLeft;
    setStatus('AKTİF - ATEŞ SERBEST', 'is-live');
...
async function endGame() {
    ...
    let timePlayed = Date.now() - gameStartTime;

    // Skoru doğrudan Supabase'e değil, Edge Function'a gönder
    // Tüm doğrulama sunucu tarafında yapılır
    let submitResult;
    try {
        const response = await fetch(`${SUPABASE_URL}/functions/v1/bright-processor`, {
            method: 'POST',
            headers: {
        ...
        console.log('[DEBUG] score-submit cevabı:', submitResult);
    } catch (e) {
        submitResult = { success: false, error: 'Bağlantı hatası.' };
        console.log('[DEBUG] score-submit isteği patladı:', e);
    }
    ...
    const { data: top3Data, error: top3Error } = await supabaseClient
        ...
        .limit(3);
    console.log('[DEBUG] top3Data:', top3Data, 'top3Error:', top3Error, 'benim skorum:', score);

    let resultMsg = `Süre bitti!\n\nSkorun: ${score}\nİsabet Oranın: %${finalAccuracy}\nEn Yüksek Seri: ${maxCombo}x\n\n`;

    if (!submitResult.success) {
        // Sunucu skoru reddettiyse
        failSound.currentTime = 0;
        failSound.play().catch(e => console.log(e));
        setTimeout(() => alert(resultMsg + `⚠️ Skor kaydedilemedi: ${submitResult.error}`), 500);
        return;
    }
...
async function getLeaderboard() {
    ...
    if (error) {
        console.log('[DEBUG] leaderboard okuma hatası:', error);
        return;
    }
    console.log('[DEBUG] leaderboard verisi:', data);
    ...
    data.forEach((row, index) => {
        ...
        li.innerHTML = `
            <div class="rank-name">
                ...
                <span class="player-username">${escapeHtml(row.username)}</span>
            </div>
...
// F12 ve geliştirici kısayollarını engelleme
